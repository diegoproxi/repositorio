...
<a name="inicio"></a>
  ...
  <label>Pantillas guardadas</label>
                  ...
                  <asp:ImageButton ID="ImageButton1" runat="server" 
                      ImageUrl="./images/uncheck.png" 
                      OnClientClick="if(document.forms[0].ddlPlantillas.value==-1){return false;} if(!confirm('Está seguro que desea eliminar la plantilla?')){return false;} return true;" 
...
<div class="clear"></div>
            ...
            <%
                ...
                parametros += "   <select name='diccionario' id='diccionario' class='listas-exportar'><option value='-1' ></option>";
                DSRepositorioIndicadoresTableAdapters.DiccionarioIndicadoresDAO dicDAO = new DSRepositorioIndicadoresTableAdapters.DiccionarioIndicadoresDAO();
                DSRepositorioIndicadores.DiccionarioIndicadoresVODataTable dicdt = dicDAO.GetDiccionario();

                for (int j = 0; j < dicdt.Count; j++)
                {
                    DSRepositorioIndicadores.DiccionarioIndicadoresVORow regVO = dicdt[j];
                    if (GetValor("diccionario") == regVO.ID.ToString())
                ...
                parametros += "<tr><td><input name='chkatrcat' type='checkbox' " + GetValorChk("chkatrcat") + " /><label class='lb-exportar'>Categoría</label>  </td><td>";

                parametros += "   <select name='categoria' id='categoria' class='listas-exportar'><option value='-1' ></option>";
                ...
                for (int j = 0; j < catdt.Count; j++)
                {
                    DSRepositorioIndicadores.CategoriasVORow regVO = catdt[j];
                    if (GetValor("categoria") == regVO.ID.ToString())
                        parametros += "<option value='" + regVO.ID + "' selected='selected'>" + regVO.DSNOMBRE + "</option>";
                    parametros += "<option value='" + regVO.ID + "'>" + regVO.DSNOMBRE + "</option>";
                }
                ...
                parametros += " </td><td><select name='ochkatrcat' id='ochkatrcat'>" + ordernesa + "</select>  <script>document.forms[0].ochkatrcat.value='" + GetValor("ochkatrcat") + "';</script> </td></tr>";
              ...
              //  , "ID", "DSNOMBRES");
                ...
                parametros += "   <select name='responsable' id='responsable' class='listas-exportar'><option value='-1' ></option>";


                DSRepositorioIndicadores.UsuariosVODataTable dtresp = usuDAO.GetUsuariosResponsables(com.triario.repositorio.util.Utils.GetUsuario(Session, Response).IDREGION);
             

                for (int i = 0; i < dtresp.Rows.Count; i++)
                {
                    DSRepositorioIndicadores.UsuariosVORow regVO = dtresp[i];
                    if (GetValor("responsable") == regVO.ID.ToString())
                        parametros += "<option value='" + regVO.ID + "' selected='selected'>" + regVO.DSNOMBRES + "</option>";
                    parametros += "<option value='" + regVO.ID + "'>" + regVO.DSNOMBRES + "</option>";
                }
